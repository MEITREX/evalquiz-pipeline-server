    ...
    async def run(self) -> AsyncIterator[PipelineStatus]:
        ...
        for pipeline_module in self.pipeline.pipeline_modules:
            ...
            try:
                ...
                yield self._build_pipeline_status(pipeline_module, ModuleStatus.FAILED, None, str(e))
                return
            input = output
        last_pipeline_module = self.pipeline.pipeline_modules[-1]
        yield self._build_pipeline_status(last_pipeline_module, ModuleStatus.SUCCESS, output)

    def _build_pipeline_status(self, pipeline_module: PipelineModule, module_status: ModuleStatus, result: Optional[Any] = None, error_message: Optional[str] = None) -> PipelineStatus:
